    ...
    stages {
            ...
            steps {
                script {
                    def response = httpRequest(
                        url: VAULT_URL + '/api/vault/cicd/reactservice/e3',
                        httpMode: 'GET',
                        customHeaders: [
                    ...
                    writeFile file: 'reactservice.env', text: envFileContent
        ...
        stage("Deploy & Health Check") {
            steps {
                script {
                    try {
                        bat "docker compose --env-file reactservice.env up --build -d"
